...
| `OPENCLAW_OWNER_ALLOW_FROM` | Optional comma-separated explicit owners for owner-only channel commands, such as `telegram:123456789,slack:U12345678`. Wildcards are rejected because webhook signatures authenticate the platform, not the human sender. When unset, normal channel chat remains available while owner-only commands fail closed. |
