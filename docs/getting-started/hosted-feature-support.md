...
| Cron and scheduled jobs | Wake a sleeping sandbox for scheduled OpenClaw jobs without copying job definitions into the host. | OpenClaw can schedule jobs and exposes reconciled/changed plugin hooks. | `experimental` | `vercel-openclaw` | Bounded earliest-wake hook projection (up to 4,096), sanitized Redis CAS state, token-revalidating Workflow, stale-workflow no-op, and watchdog anti-entropy tests. | Require an exact verified bundle declaring legacy `cron-projection-v1` or reconciled `cron-projection-v2`, then verify schedule, reschedule, delete, sleep, and wake end to end before marking supported. |
