...
Required capabilities are `admin-http-rpc-v1`, `gateway-suspend-v1`, `telegram-durable-ack-v1`, and either `cron-projection-v1` or `cron-projection-v2`; required plugin IDs include `admin-http-rpc`, `slack`, and `telegram`. Cron v1 preserves the legacy `gateway_start` baseline, while v2 attests the complete `cron_reconciled` hook contract. Hosted code gates these paths on admitted capability identity, never release-version guesses.

The sandbox downloads only the manifest-named canonical tarball, verifies its exact bytes and SHA-256, verifies every contained asset before extraction, and installs the verified Slack tarball offline through the OpenClaw plugin installer. It retains that digest-pinned archive as the immutable comparison source. The bundle identity receipt is written only after config, external-plugin installation, and the runtime's reported OpenClaw version all match the admitted manifest. Persistent resume requires the receipt and stored metadata to match exactly, then compares the installed entrypoint, dependencies, bundled plugins, external Slack package, and Control UI with the sealed archive before Gateway launch. A missing or modified local archive is recovered only from the same digest-pinned release asset. An existing sandbox with missing or stale identity fails closed with `OPENCLAW_BUNDLE_MIGRATION_REQUIRED`; the dashboard never deletes sandbox-owned cron or other state as an implicit upgrade step.
